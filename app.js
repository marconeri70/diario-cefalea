/* =========================
   ...
   - Intestazione stampa con logo (assets/ptv.png) via URL assoluto (GitHub Pages)
   ========================= */
...
/* Logo intestazione per stampa/PDF (IMPORTANTE: URL assoluto perché la stampa apre about:blank) */
const PTV_LOGO_URL = "https://marconeri70.github.io/diario-cefalea/assets/ptv.png";
...
function startEdit(id) {
  ...
  if (stress) {
    stress.value = String(a.stress ?? 0);
  }
  if (stressVal) {
    stressVal.textContent = String(a.stress ?? 0);
  }
...
function buildPrintHTML(yyyyMM) {
  ...
  const printCSS = `
    ...
    .ptv-head{ display:flex; justify-content:space-between; align-items:center; gap:12px; margin-bottom: 8px; }
    .ptv-left{ display:flex; align-items:center; gap:10px; min-width: 0; }
    .ptv-logo{ height: 12mm; max-height: 12mm; width:auto; max-width: 70mm; object-fit: contain; }
...
if (form) {
  form.addEventListener("submit", (ev) => {
    ...
    const sleepHoursVal =
      el("sleepHours")?.value === "" || el("sleepHours") == null ? null : Number(el("sleepHours").value);
